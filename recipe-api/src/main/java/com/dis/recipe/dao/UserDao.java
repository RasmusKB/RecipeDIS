...
public interface UserDao extends JpaRepository<User, String>{
    ...
    @Query(value = "SELECT * FROM Userdis WHERE username = ?1 OR email = ?2", nativeQuery = true)
    List<User> findUserByUsernameOrEmail(String username, String email);

    @Query(value = "SELECT * FROM Userdis WHERE username = ?1 AND password = ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM Userdis WHERE id = ?1 AND username = ?2 AND password = ?3", nativeQuery = true)
